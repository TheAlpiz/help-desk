...
notificationRouter.patch("/:id/read", zValidator("json", markNotificationReadSchema), async (c) => {
  const user = c.get("user");
...
notificationRouter.put("/preferences", zValidator("json", updateNotificationPreferenceSchema), async (c) => {
  const user = c.get("user");
  const data = c.req.valid("json");
  try {
    let pref;
