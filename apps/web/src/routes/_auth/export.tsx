...
const ENTITY_ICONS: Record<ExportEntity, React.ReactNode> = {
  tickets: <FileText className="w-4 h-4" />,
  tasks: <Table2 className="w-4 h-4" />,
  users: <Table2 className="w-4 h-4" />,
  audit_logs: <FileText className="w-4 h-4" />,
  sla_reports: <Table2 className="w-4 h-4" />,
};
...
function StatusBadge({ status }: { status: ExportStatus }) {
  ...
  const map = {
    pending: { cls: "text-amber-400 bg-amber-400/10", icon: <Clock className="w-3 h-3 animate-spin" />, label: t("status.pending") },
    done: { cls: "text-emerald-400 bg-emerald-400/10", icon: <CheckCircle2 className="w-3 h-3" />, label: t("status.done") },
    error: { cls: "text-red-400 bg-red-400/10", icon: <AlertCircle className="w-3 h-3" />, label: t("status.error") },
  }[status];
  return (
...
function ExportCenter() {
  ...
  const entities = (["tickets", "tasks", "users", "audit_logs", "sla_reports"] as ExportEntity[]);
  const formats = (["csv", "json", "xlsx"] as ExportFormat[]);
  ...
  const requestExport = async (entity: ExportEntity, format: ExportFormat) => {
    ...
    try {
      ...
      success(t("toast.success", { label: t(`entities.${entity}.label`), format: format.toUpperCase() }));
    } catch (err: any) {
      setJobs((j) =>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-surface-container border border-outline-variant rounded-xl p-4 space-y-3">
        <h3 className="text-xs font-semibold text-on-surface-variant uppercase tracking-wide">{t("dateRange.label")}</h3>
        <div className="flex items-center gap-3">
          <div className="flex flex-col gap-1 flex-1">
            <label className="text-[11px] text-on-surface-variant">{t("dateRange.from")}</label>
            <input
              type="date"
              value={dateFrom}
          ...
          <div className="flex flex-col gap-1 flex-1">
            <label className="text-[11px] text-on-surface-variant">{t("dateRange.to")}</label>
            <input
              type="date"
              value={dateTo}
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-on-surface">{t(`entities.${entity}.label`)}</p>
                  <p className="text-xs text-on-surface-variant/60 mt-0.5">{t(`entities.${entity}.description`)}</p>
                </div>
              </div>
              <div className="flex items-center gap-2 flex-wrap">
                {formats.map((fmt) => (
                  <button
                    ...
                    disabled={!!pendingJob}
                    aria-label={t("ariaExport", { label: t(`entities.${entity}.label`), format: fmt.toUpperCase() })}
      ...
      {jobs.length > 0 && (
        <div className="space-y-2">
          <h3 className="text-xs font-semibold text-on-surface-variant uppercase tracking-wide">{t("history")}</h3>
              ...
              <div key={job.id} className="flex items-center gap-3 px-4 py-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-on-surface">
                    {t(`entities.${job.entity}.label`)} · {t(`formats.${job.format}`)}
                  </p>
