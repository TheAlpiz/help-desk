...
function PricingPage() {
  ...
  const faqs = t("pricing.faq.items", { returnObjects: true }) as {
    q: string;
    a: string;
  }[];
  const starterFeatures = t("pricing.plans.starter.features", {
    returnObjects: true,
  }) as string[];
  const proFeatures = t("pricing.plans.pro.features", {
    returnObjects: true,
  }) as string[];
  const enterpriseFeatures = t("pricing.plans.enterprise.features", {
    returnObjects: true,
  }) as string[];
  const compareRows = t("pricing.compare.rows", { returnObjects: true }) as {
    label: string;
    s?: string;
    p?: string;
    e?: string;
  ...
  const compareData = [
    {
      label: compareRows[0]?.label,
      s: compareRows[0]?.s ?? "3",
      p: compareRows[0]?.p ?? "5 (base)",
      e: compareRows[0]?.e ?? "Custom",
    },
    {
      label: compareRows[1]?.label,
      s: compareRows[1]?.s ?? "500",
      p: compareRows[1]?.p ?? "Unlimited",
      e: compareRows[1]?.e ?? "Unlimited",
    },
    {
      label: compareRows[2]?.label,
      s: compareRows[2]?.s ?? "1",
      p: compareRows[2]?.p ?? "10",
      e: compareRows[2]?.e ?? "Unlimited",
    },
        ...
        <section className="px-6 md:px-10 pb-24 max-w-[1280px] mx-auto relative z-10">
          <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto items-center">
            ...
            <div className="rounded-2xl border border-white/10 bg-surface-container/50 backdrop-blur-sm p-8 flex flex-col h-full hover:border-white/20 transition-colors">
              ...
              <Link
                to="/register"
                className="w-full text-center px-6 py-3 bg-surface-container-high hover:bg-surface-bright rounded-xl font-semibold transition-all mb-8"
              >
                {t("pricing.plans.starter.cta")}
              </Link>
              <div className="space-y-4 flex-grow">
                <p className="text-sm font-semibold text-on-surface">
                  {t("pricing.plans.starter.includesLabel")}
                </p>
            ...
            <div className="rounded-2xl border border-primary bg-surface-container/80 backdrop-blur-md p-8 flex flex-col h-[105%] relative shadow-[0_0_40px_rgba(var(--color-primary),0.15)] z-10">
              ...
              <Link
                to="/register"
                className="w-full text-center px-6 py-3 bg-primary text-on-primary hover:bg-primary/90 hover:shadow-[0_0_20px_rgba(var(--color-primary),0.4)] rounded-xl font-semibold transition-all active:scale-95 mb-8"
              >
                {t("pricing.plans.pro.cta")}
              </Link>
              <div className="space-y-4 flex-grow">
                <p className="text-sm font-semibold text-on-surface">
                  {t("pricing.plans.pro.includesLabel")}
                </p>
                <ul className="space-y-3">
            ...
            <div className="rounded-2xl border border-white/10 bg-surface-container/50 backdrop-blur-sm p-8 flex flex-col h-full hover:border-white/20 transition-colors">
              ...
              <Link
                to="/contact"
                className="w-full text-center px-6 py-3 border border-white/20 hover:bg-white/5 rounded-xl font-semibold transition-all mb-8"
              >
                {t("pricing.plans.enterprise.cta")}
              </Link>
              <div className="space-y-4 flex-grow">
                <p className="text-sm font-semibold text-on-surface">
                  {t("pricing.plans.enterprise.includesLabel")}
                </p>
                <ul className="space-y-3">
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-white/10">
                    <th className="py-4 font-semibold text-on-surface w-1/3">
                      {t("pricing.compare.featureCol")}
                    </th>
                    <th className="py-4 font-semibold text-center w-2/9">
                      {t("pricing.plans.starter.name")}
                    </th>
                    <th className="py-4 font-semibold text-center text-primary w-2/9">
                      {t("pricing.plans.pro.name")}
                    </th>
                    <th className="py-4 font-semibold text-center w-2/9">
                      {t("pricing.plans.enterprise.name")}
        ...
        <section className="px-6 md:px-10 py-24 max-w-[800px] mx-auto">
          ...
          <div className="space-y-6">
            {faqs.map((faq, i) => (
              <div
                key={i}
                className="p-6 rounded-2xl bg-surface-container border border-white/5"
              >
