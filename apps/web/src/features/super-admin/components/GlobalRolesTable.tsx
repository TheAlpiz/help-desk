...
function CreateGlobalRoleModal({ onClose }: { onClose: () => void }) {
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      const res = await api.roles.global.$post({ json: { name: form.name, description: form.description } });
      const roleBody = await res.json() as any;
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["global-roles"] }); onClose(); },
    onError: (err: Error) => setError(err.message),
  });

  const textareaCls = "w-full px-3 py-2 bg-surface-container-high border border-outline-variant rounded-lg text-sm text-on-surface placeholder:text-on-surface-variant/40 focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary/60 transition-colors resize-none";
...
function RolePermissionsDrawer({ role, onClose }: { role: GlobalRole; onClose: () => void }) {
  ...
  const { data: permResponse, isLoading } = useQuery({
    queryKey: ["global-role-permissions", role.id],
    queryFn: async () => {
...
function DeleteRoleModal({ role, onClose }: { role: GlobalRole; onClose: () => void }) {
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      const res = await api.roles.global[":id"].$delete({ param: { id: role.id } });
      if (!res.ok) {
        const body = await res.json() as any;
        throw new Error(body?.error?.message || body?.message || "Failed to delete role");
      }
    },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["global-roles"] }); onClose(); },
  });
...
export function GlobalRolesTable() {
  ...
  const { data: response, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      const res = await api.roles.global.$get();
      const body = await res.json() as any;
      return body as { data: GlobalRole[] };
    },
  });

  const roles: GlobalRole[] = response?.data ?? [];

  return (
    <>
      {showCreate && <CreateGlobalRoleModal onClose={() => setShowCreate(false)} />}
      {editRole && <RolePermissionsDrawer role={editRole} onClose={() => setEditRole(null)} />}
